...
class HomeController{
  ...
  void navigateToCorrectScreen(BuildContext context) async
  {
    ...
    if(list.isEmpty)
      ...
      if(Common.convertStringToTimeOfDay(list[0]['Slot_time_start']).hour == 10)
      {
        Navigator.push(context, MaterialPageRoute(builder: (context) => ViewWalkInScreen('assets/images/Walk_in_ticket.png')),);
      }
      else if(Common.convertStringToTimeOfDay(list[0]['Slot_time_start']).hour == 13)
      {
  ...
  deleteRecordOfWalkInUser()
  {
    ...
    if(!(DateTime.now().isAfter(Common.convertTimeOfDayToDateTime(TimeOfDay(hour: 16, minute: 5))) && DateTime.now().isBefore(Common.convertTimeOfDayToDateTime(TimeOfDay(hour: 1, minute: 0)))))
      {
        print('Inside: if (Delete record if the time is between 4:05 PM to 12:00 AM) ');
        dbManager.delete(Constant.mobile_user_details, 'id = ?', [1]);
      }
    else
      {
        print('Inside: else (Delete record if the time is between 4:05 PM to 12:00 AM) ');
      }
  }
